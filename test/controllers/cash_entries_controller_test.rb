...
class CashEntriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create cash_entry' do
    assert_difference('CashEntry.count') do
      post cash_entries_url,
           params: { cash_entry: { amount: @cash_entry.amount, description: @cash_entry.description, type: @cash_entry.type,
                                   user_id: @cash_entry.user_id } }
    end
  ...
  test 'should update cash_entry' do
    patch cash_entry_url(@cash_entry),
          params: { cash_entry: { amount: @cash_entry.amount, description: @cash_entry.description, type: @cash_entry.type,
                                  user_id: @cash_entry.user_id } }
